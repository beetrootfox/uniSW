 	...
 	public static void main(String args []) {    
		    ...
		    for (int i = 0; i < nick.length; i++) {
		      ...
		      for (int j = 2; j < nick[i].length; j=j+2) {
			    ...
			    //GeneralSearchComparator comp = new GeneralSearchComparator(true); //uncomment the previous comment to use DepthFirst
			    GeneralSearchComparator comp = new GeneralSearchComparator(false); //uncomment the prevoius comment to use BreadthFirst
			    
			    ilist.IList<Node<Coordinate>> path = finder.generalSearch(nicksGraph.nodeWith(new Coordinate (9, 0)), 
			    		nicksGraph.nodeWith(new Coordinate (9, 0)), new Distance<Coordinate>(){

							@Override
							public double calculate(Coordinate pos1,
									Coordinate pos2) {

								//return Math.abs(pos1.x - pos2.x) + Math.abs(pos1.y - pos2.y);
								return 0;    //uncomment the previous comment if you are not using A*
							}
